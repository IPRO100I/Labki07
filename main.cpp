...
public:
    ...
    bool CheckForAllColisions(std::string Direction){
        for(auto Object : DrawableCollection){
            if(dynamic_cast<Platform*>(Object)){
                ...
                {
                    if(Direction == "Bottom"){
                        while(getGlobalBounds().intersects(Object->getGlobalBounds())){
                            move(0,-0.5);
                        }
                        TouchingGround = true;
                        return true;
    ...
    void Step(sf::Time TimeElapsed){
        ...
        std::cout<<GetHorizontalColisionBorder().first << ":" <<GetHorizontalColisionBorder().second <<std::endl;
...
int main()
{
    ...
    if(Texture.loadFromFile("Images\\Map\\sea.png")){
        Texture.setRepeated(true);
        TextureCollection.push_back(Texture);
    }

    //Background sprite
    sf::Sprite Background(TextureCollection[2]);
    Background.setTextureRect(sf::IntRect(0,0,800,600));

    DrawableCollection.push_back(&Background);
    ...
    CharacterSprite.AddAnimationFrame(sf::IntRect(15,0,35,37));
    CharacterSprite.AddAnimationFrame(sf::IntRect(65,0,35,37));
    CharacterSprite.AddAnimationFrame(sf::IntRect(115,0,35,37));
    ...
    //Creating Platforms
    for(int i = 0 ; i < 6 ; i++){
        DrawableCollection.push_back(new Platform(TextureCollection[1],sf::IntRect(41, 24, 98, 76)));
    }

    DrawableCollection[2]->setPosition(0,600);
    DrawableCollection[3]->setPosition(150,500);
    DrawableCollection[4]->setPosition(300,400);
    DrawableCollection[5]->setPosition(450,300);
    DrawableCollection[6]->setPosition(600,200);
    DrawableCollection[7]->setPosition(750,100);
